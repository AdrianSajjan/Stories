    ...
    [
      check('name')
        ...
        .custom(async (value, { req }) => {
          ...
          if (user && user.name === value.trim()) throw new Error('Old and new name cannot be same')
          return true
        })
    ]
  ],
    ...
    [
      check('email')
        ...
        .custom(async (value, { req }) => {
          ...
          if (user) {
            if (user._id == req.user.id) throw new Error('New and old email cannot be same')
    ...
    [
      ...
      check('newPassword')
        ...
        .custom(async (value, { req }) => {
          ...
          if (user) {
            const isMatch = await bcrypt.compare(value, user.password)
            if (isMatch) throw new Error('New password and old password cannot be same')
          }
          return true
        }),
 ...
 * @access : Private
 */
router.delete('/', [auth, [check('password').not().isEmpty().withMessage('Password cannot be empty')]], async (req, res) => {
  const errors = validationResult(req)
  if (!errors.isEmpty())
    return res.status(400).json({
      type: VALIDATION,
  ...
  try {
    ...
    if (likedPost.length > 0) {
      ...
      const DeleteAllLikesFromPosts = async () => {
        ...
        console.log(`Deleting associated likes of user: ${userID} from post: ${post._id}`)
    ...
    if (commentedPost.length > 0) {
      ...
      const DeleteAllCommentsFromPosts = async () => {
        ...
        console.log(`Deleting associated comments of user: ${userID} from post: ${post._id}`)

        post.comments = post.comments.filter((comment) => comment.user != userID)
        await post.save()
    ...
    if (followingProfile.length > 0) {
      ...
      const DeleteAllFollowing = async () => {
        ...
        console.log(`Deleting associated following of user: ${userID} from post: ${post._id}`)

        profile.following = profile.following.filter((follow) => follow.user != userID)
        await profile.save()
    ...
    if (followerProfile.length > 0) {
      ...
      const DeleteAllfollower = async () => {
        ...
        console.log(`Deleting associated followers of user: ${userID} from post: ${post._id}`)

        profile.followers = profile.followers.filter((follow) => follow.user != userID)
        await profile.save()
