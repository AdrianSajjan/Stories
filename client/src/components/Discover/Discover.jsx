...
const Discover = ({ discoverProfiles, loadDiscoverProfiles }) => {
  ...
  const { profiles, loading: profilesLoading, endOfProfiles, currentPage: page } = discoverProfiles
    ...
    return (
      <Fragment>
        {endOfProfiles ? (
          ...
          <Button color="info" className="mx-auto d-block mb-1" onClick={loadProfiles}>
            Load More
          </Button>
        )}
      </Fragment>
    ...
    return (
      <Fragment>
        {profiles.map((profile) => (
          <ProfileCard key={profile._id} profile={profile} isDismissable={true} />
        ))}
      </Fragment>
    )
  }
    ...
    <Fragment>
      <div ref={rootRef} className="side-area-container" style={{ top: -top }}>
        <div className="side-area-card">
          <p className="side-area-card-title text-center text-secondary">Discover Profiles</p>
          <hr />
          <ProfileCards />
          <DiscoverBtn />
        </div>
