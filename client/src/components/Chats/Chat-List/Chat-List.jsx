...
const ChatList = ({ chat }) => {
  ...
  const getFormattedMessage = () => {
    const sender =
      messages[messages.length - 1].sender.user === receiver.user
        ? receiver.username
        : 'You'
  ...
  const getProfileImage = () => {
    if (receiver.avatar && receiver.avatar.url && receiver.avatar.url.length)
  ...
  return (
    <Link to={`/home/chats/${receiver.user}`} className="chat-list py-2">
      ...
      <div className="d-flex flex-column ml-4 align-items-start justify-content-center">
        <p className="text-secondary font-weight-bold leading-1 m-0">
          @{receiver.username}
        </p>
...
const ChatLists = ({ allChats }) => {
  ...
  if (!chats.length && loading)
    return <Spinner color="primary" className="d-block mt-4 mx-auto" />

  if (!chats.length && !loading)
    return <h3 className="text-center text-muted mt-4">No Chats Found</h3>

  return (
    <div className="side-area-container chat-list-container">
      <p className="lead text-center text-muted">All Chats</p>
