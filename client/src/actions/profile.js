...
export const setProfile = (data) => async (dispatch) => {
  try {
    ...
    if (data && data.type) {
      if (data.type === 'VALIDATION') dispatch({ type: PROFILE_ERROR, payload: data.errors })
      else toast.error(data.msg || "Coudn't create profile")
    } else {
...
export const loadDiscoverProfiles = (page) => async (dispatch) => {
  ...
  try {
    ...
    if (res.data.length > 0) dispatch({ type: SET_DISCOVER_PROFILES, payload: res.data })
